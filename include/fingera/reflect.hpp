...
class member_getter_setter {
 public:
  using type = ExportType;
  static inline ExportType get(const ClassType &object) {
    return object.*pointer;
  }
  static inline void set(ClassType &object, ExportType &&value) {
...
#define FR_MEMBER_FULL(name, export_type, export_name)                   \
  {                                                                      \
...
#define FR_MEMBER_BASE(name) \
  FR_MEMBER_FULL(name, decltype((static_cast<type *>(nullptr))->name), name)

#define FR_MEMBER_NAME(name, new_name) \
  FR_MEMBER_FULL(name, decltype((static_cast<type *>(nullptr))->name), new_name)

#define FR_MEMBER_TYPE(name, export_type) \
  FR_MEMBER_FULL(name, export_type, name)

#define FR_MEMBER_FUNC(name, export_type, getter, setter)                    \
  {                                                                          \
...
#define FR_SUPER_CLASS(base) fingera::reflector<base>::visit<Visitor>(v);

#define FINGERA_REFLECT_BEGIN(T)       \
  namespace fingera {                  \
  template <>                          \
  class reflector<T> {                 \
   public:                             \
